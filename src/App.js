...
function App() {
  ...
  const addToCart = (product) => {
    ...
    } else if (product.tipo === 'tocadisco') {
  ...
  const addToWishlist = (product) => {
    ...
    if (product.tipo === 'vinilo') {
      alert(`${product.title} ha sido agregado al wishlist!`);
    } else if (product.tipo === 'tocadisco') {
      alert(`${product.name} ha sido agregado al wishlist!`);
    }
  };
  ...
  style={{ transition: 'color 0.3s', color: '#f8f9fa' }}
